...
class Client extends EventEmitter
{
    constructor(id, token, options)
    {
        ...
        this._interval = null;
        this._timer = ((options.interval && options.interval >= 3600000) ? options.interval : 3600000);
    ...
    _postStats()
    {
		...
		fetch(`https://disforge.com/api/botstats/${this._bot._id}`, {
        method: 'POST',
	...
	.then(json => {
		...
		if(json.error){
			...
			this.emit("debug", "[\x1b[32mSuccess\x1b[0m] Stats posted to \x1b[36mDisforge!\x1b[0m !");
			
			this.emit("post");

	})
	.catch((err) => {
			this.emit("debug", "[\x1b[31mError\x1b[0m] An error has occurred while updating stats on \x1b[36mDisforge\x1b[0m !");
